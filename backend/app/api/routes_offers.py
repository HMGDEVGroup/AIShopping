...
async def offers(
    ...
    try:
        ...
        # ✅ FIX: shopping_search may be async in your project
        if inspect.isawaitable(results):
            results = await results

        if not isinstance(results, dict):
            raise Exception(f"shopping_search returned non-dict: {type(results)}")
    ...
    for r in raw_offers:
        ...
        # ✅ Drop rows that are missing required fields for OfferItem
        # (title is required; source/link can be None but Pydantic may enforce based on schema)
        row = {
            "title": str(title),
            "price": price_str,
        ...
        # Validate safely: skip anything that fails OfferItem
        try:
            OfferItem(**row)
