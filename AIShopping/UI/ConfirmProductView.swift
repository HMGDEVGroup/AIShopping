...
struct ConfirmProductView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Confirm Product").font(.title2).bold()
    ...
    private func fetchOffers() async {
        ...
        do {
            let offers = try await state.api.offers(canonicalQuery: c.canonical_query, includeMembership: state.includeMembershipStores)
